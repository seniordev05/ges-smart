<div class="modal-dialog">
	<div class="modal-content">
		<div class="modal-header">
			<button type="button" class="close" data-dismiss="modal">&times;</button>
			<h4 class="modal-title"><?=lang('create_expense')?></h4>
		</div>
		<?php $attributes = array('class' => 'bs-example form-horizontal','id' => 'createExpenseForm'); echo form_open_multipart(base_url().'budget_expenses/create',$attributes); ?>
			<div class="modal-body">
				<div class="form-group">
					<label class="col-lg-4 control-label"><?=lang('amount')?> <span class="text-danger">*</span></label>
					<div class="col-lg-8">
						<input type="text" class="form-control expense_amount" placeholder="800.00" name="amount">
					</div>
				</div>
				<div class="form-group">
					<label class="col-lg-4 control-label"><?=lang('notes')?></label>
					<div class="col-lg-8">
						<textarea class="form-control ta" name="notes"></textarea>
					</div>
				</div>
				
				<div class="form-group">
					<label class="col-lg-4 control-label"><?=lang('expense_date')?> <span class="text-danger">*</span></label>
					<div class="col-lg-8">
						<input class="datepicker-input form-control" type="text" value="<?=strftime(config_item('date_format'), time());?>" name="expense_date" data-date-format="<?=config_item('date_picker_format');?>" >
					</div>
				</div>

				<div class="form-group" style="display: flex; justify-content: space-between;">
					<label class="radio-inline">
						<input type="radio" name="expense_type" class="ExpenseType" value="item">Item
					</label>
					<label class="radio-inline">
						<input type="radio" name="expense_type" class="ExpenseType" value="category" checked>Category
					</label>
				</div>

				<div class="form-group ExpenseCategory" style="display: none;">
					<label class="col-lg-4 control-label"><?=lang('category')?> <span class="text-danger">*</span></label>
					<div class="col-lg-8">
						<select name="category" class="form-control m-b" id="main_category">
							<option value="" disabled selected>Choose Category</option>
							<?php foreach ($categories as  $cat) { ?>
							<option value="<?php echo $cat['cat_id']?>"><?php echo $cat['category_name']?></option>
							<?php } ?>
						</select>
					</div>
				</div>
				<div class="form-group ExpenseSubCategory" style="display: none;">
					<label class="col-lg-4 control-label"><?=lang('sub_category')?> <span class="text-danger">*</span></label>
					<div class="col-lg-8">
						<select name="sub_category" class="form-control m-b" id="sub_category">
							<option value="" >Choose Sub-Category</option>
						</select>
					</div>
				</div> 

				<!-- customize budget expenses start-->
				
				<div class="form-group ExpenseItem" style="display: none;">
					<label class="col-lg-4 control-label"><?=lang('item')?> <span class="text-danger">*</span></label>
					<div class="col-lg-8">
						<select name="item" class="form-control item_selected">
							<option value="">Choose Items</option>
							<?php foreach (Invoice::saved_items() as $key => $item) { ?>
							<option value="<?=$item->item_id?>"><?=$item->item_name?> - <?=$item->unit_cost?></option>
							<?php } ?>					
						</select>
					</div>
				</div>

				<div class="form-group ExpenseQty" style="display: none;">
					<label class="col-lg-4 control-label">QTY <span class="text-danger">*</span></label>
					<div class="col-lg-8">
						<input type="number" class="form-control item_qty" name="item_qty">
					</div>
				</div>

				<!-- customize budget expenses end -->
				
				<div class="form-group">
					<label class="col-lg-4 control-label"><?=lang('attach_file')?></label>
					<div class="col-lg-8">
						<label class="btn btn-default btn-choose">Choose File</label>
						<input type="file" class="form-control" data-buttonText="<?=lang('choose_file')?>" data-icon="false" data-classButton="btn btn-default" data-classInput="form-control inline input-s" name="receipt">
					</div>
				</div>
			</div>
			<div class="modal-footer"> <a href="#" class="btn btn-danger" data-dismiss="modal"><?=lang('close')?></a>
				<button type="submit" class="btn btn-success" id="accountsCreateExpense"><?=lang('save_changes')?></button>
			</div>
		</form>
	</div>
</div>
<script type="text/javascript">
		$('.datepicker-input').datepicker({ language: locale});
		$(document).ready(function(){
			var radioValue = $("input[type='radio']:checked").val();
			if(radioValue == 'item'){
					$('.ExpenseItem').css('display','block');
					$('.ExpenseQty').css('display','block');
					$('.ExpenseCategory').css('display','none');
					$('.ExpenseSubCategory').css('display','none');
			}

			if(radioValue == 'category'){
					$('.ExpenseItem').css('display','none');
					$('.ExpenseQty').css('display','none');
					$('.ExpenseCategory').css('display','block');
					$('.ExpenseSubCategory').css('display','block');
			}
    });
		$(document).on("change", ".ExpenseType", function() {
			var expense_type = $(this).val();
			if(expense_type == 'item'){
					$('.ExpenseItem').css('display','block');
					$('.ExpenseQty').css('display','block');
					$('.ExpenseCategory').css('display','none');
					$('.ExpenseSubCategory').css('display','none');

					$('.expense_amount').change(function() {
						var expense_amount = $(this).val();
						var item_id = $(".item_selected").val();
						$.ajax({
							type: "POST",
							url: base_url + 'invoices/items/get_item_unit_cost',
							data:  {item_id:item_id},
							success: function (data) {       
								$(".item_qty").val(Math.floor(expense_amount / data));                         
							}
						});
					});

			}

			if(expense_type == 'category'){
					$('.ExpenseItem').css('display','none');
					$('.ExpenseQty').css('display','none');
					$('.ExpenseCategory').css('display','block');
					$('.ExpenseSubCategory').css('display','block');
			}
		});
		$(document).on("change", ".item_qty", function() {
			var item_qty = $(this).val();
			var item_id = $(".item_selected").val();
			$.ajax({
					type: "POST",
					url: base_url + 'invoices/items/get_item_unit_cost',
					data:  {item_id:item_id},
					success: function (data) {       
						$(".expense_amount").val(item_qty * data);                                  
					}
			});
		});
</script>
