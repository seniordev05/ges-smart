...
class Budget_expenses extends MX_Controller {
	...
	function index()
	{
		...
		// echo '<pre>'; print_r($data['dgt_payments']);
// echo $data['dgt_payments']['amount'];
		 // exit;
		$this->template
	...
	function create()
	{
		...
		// foreach (Invoice::saved_items_where() as $key => $item) 
		// Item::list_items() as $key => $item
        ...
        $this->db->select_max('po_code', 'max');
  			$query = $this->db->get('budget_expenses');
				...
				if($expense_id = App::save_data('budget_expenses',$data)){
					...
					$unit_quantity=$this->input->post('item_qty');
