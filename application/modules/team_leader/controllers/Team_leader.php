...
class Team_leader extends MX_Controller
{
    public function __construct()
    {
        ...
        foreach($all_routes as $key => $route){
            if($route == 'team_leader'){
                $routname = team_leader;
            } 
        }
        if(empty($routname)){
             $this->session->set_flashdata('message', lang('access_denied'));
    ...
    public function index()
    {
        ...
        $this->template->title(lang('team_leader').' - '.config_item('company_name'));
        $data['page'] = lang('team_leader');
        $data['form'] = true;
        ...
        $this->template
                ->set_layout('users')
                ->build('team_leader', isset($data) ? $data : null);
    }
