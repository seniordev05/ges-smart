...
foreach ($all_grades as $grades) {
  ...
  {
    $dep_title=$this->db->get_where('dgt_departments',array('deptid'=>$des['department_id']))->result_array();
    // var_dump($dep_title[0]['deptname']);echo '<br><br>';
    ...
    if($des['avatar'] == '')
    {
      ...
      if($i == 0){
        $res = array(
          ...
          "dep_title"=>$dep_title[0]['deptname'],
   ...
   myDiagram.nodeTemplateMap.add("greenbox",  // the default category
    ...
    $(go.Panel, "Horizontal", 
      ...
      $(go.TextBlock, "Name",textStyle(), 
        ...
        $(go.TextBlock, "Dep_title", textStyle(), 
              {
              ...
              new go.Binding("text", "dep_title").makeTwoWay()), 
  ...
  function updateData() { 
    ...
    if(mem_name == '' || mem_name == 'Name'){
      alert("Please Enter Name.....");
        ...
        model.setDataProperty(data, "dep_title", dep_title);
...
function pdfdownload(){
      ...
      setTimeout(function(){
      var dataURL = $('canvas').get(1);
      console.log(dataURL);
      var doc = new jsPDF();
      doc.setFontSize(33);
      doc.setFillColor(255, 255,255,255);
      doc.addImage(dataURL, 'png', 5, 25,201, 90);
...
<section class="wrapper"> 
  <div class='col-md-12'> 
      <div class='col-md-2'>
        <button id='ZoomOut' onclick="zoomoutclick()" class='btn btn-primary btn-sm'>Zoom Out</button>
        <button id='ZoomIn' onclick="zoominclick()" class='btn btn-primary btn-sm'>Zoom In</button>
      </div>
   <?php if(User::is_admin()){  ?>
       <div class="col-sm-9">
          <a href="<?=base_url()?>organisation/send_organisation/<?=User::get_id();?>" data-toggle="ajaxModal" title="Email" class='btn btn-success'>  Email Organisation </a>
       </div>
   <?php } else{ ?>
   ...
   <?php }  ?>
      <div class='col-md-1'></div>
        <div class='col-md-1'>
            <button id='ZoomOut' onclick="pdfdownload()" class='btn btn-success'>PDF</button>
        </div>
  </div>
