                  ...
                  <div class="table-responsive border">
                    ...
                    <tbody>
                     ...
                     echo '<tr>';
                        ...
                        foreach($months as $month) {
                         ...
                         // echo $this->db->last_query();
                          ...
                          if(!empty($revenues)){
                              ...
                              echo '<td>'.Applib::format_currency($inv->currency, $revenues['total_amount'])
                              .'</td>';

                                      // Sum of Revenue in Project & Month wise                      
                         }else{
                          echo '<td>-</td>';                          
                        }
                        $i++;
                      ...
                      Applib::format_currency($inv->currency, $category_revenue)
                      .'</td>';       
                      echo '</tr>';
                ...
                // <!-----My Code---->
                      ...
                      foreach ($months as $month) {
                        if(strlen($i) == 1){
                           ...
                           $this->db->select('SUM(dgt_payments.total_sale_quantity)  as total_sale');
                           $this->db->where("DATE_FORMAT(dgt_payments.payment_date,'%Y-%m')",$financial_start_date);                             
                           $total_sales_values = $this->db->get('dgt_payments')->row_array();
                            
                          if (!empty($total_sales_values['total_sale'])) {
                            echo '<td>'.Applib::format_currency($inv->currency, $total_sales_values['total_sale']).'</td>'; 
                           }else{
                            echo "<td>-</td>";
                           }
                        $i++;
                     ...
                     // <!-----My Code---->
                      ...
                      // Generating Month  here 
                        ...
                        foreach($months as $month) {
                         ...
                         // echo $this->db->last_query();
                          ...
                          if(!empty($total_revenues['total_amount'])){
                              ...
                              $total_revenues['total_amount']=$total_revenues['total_amount']+$total_sales_values['total_sale'];
                              
                            $c_total_revenues += $total_revenues['total_amount'];
                            echo '<th>'.Applib::format_currency($inv->currency, $total_revenues['total_amount']).'</th>'; 
                        ...
                        echo '<th style = "text-align:right">'.Applib::format_currency($inv->currency, $c_total_revenues).'</th>';                       
                ...
                <!-- Budget_expenses start -->
                     ...
                     /* Getting Post Year */
                      ...
                      // Generating Month  here 
                         ...
                         // echo $this->db->last_query();
                          ...
                          if(!empty($expenses)){
                            $category_expense += $expenses['total_amount'];
                               echo '<td>'.Applib::format_currency($inv->currency, $expenses['total_amount']).'</td>'; 
                       ...
                       echo '<td style = "text-align:right">'.Applib::format_currency($inv->currency, $category_expense).'</td>'; 
                      ...
                      foreach ($months as $month) {
                        if(strlen($i) == 1){
                           ...
                           $this->db->select('SUM(dgt_payments.total_cost_quantity)  as total_cost_amt');
                           $this->db->where("DATE_FORMAT(dgt_payments.payment_date,'%Y-%m')",$financial_start_date);                             
                           $total_sales_values = $this->db->get('dgt_payments')->row_array();

                           if (!empty($total_sales_values['total_cost_amt'])) {
                            echo '<td>'.Applib::format_currency($inv->currency, $total_sales_values['total_cost_amt']).'</td>'; 
                           }else{
                            echo "<td>-</td>";
                           }
                     ...
                     echo '<tr>';
                      ...
                      // Generating Month  here 
                        foreach($months as $month) {
                         ...
                         // echo $this->db->last_query();
                          ...
                          if(!empty($total_expenses['total_amount'])){
                              ...
                              $total_expenses['total_amount']=$total_expenses['total_amount']+$total_sales_values['total_cost_amt'];
                            $c_total_expense += $total_expenses['total_amount'];
                            echo '<th>'.Applib::format_currency($inv->currency, $total_expenses['total_amount']).'</th>'; 
                            
                                      // Sum of Revenue in Project & Month wise                      
                         }
                         elseif(empty($total_expenses['total_amount']) && !empty($total_sales_values['total_cost_amt'])){
                             $total_expenses['total_amount']=$total_sales_values['total_cost_amt'];
                            $c_total_expense += $total_expenses['total_amount'];
                            echo '<th>'.Applib::format_currency($inv->currency, $total_sales_values['total_cost_amt']).'</th>'; 
                         }
                       ...
                       echo '<th style = "text-align:right">'.Applib::format_currency($inv->currency, $c_total_expense).'</th>'; 
                    ...
                    <!-- Sum of profit for each month -->
                     ...
                     //print_r($sum_revenues); exit;
                        ...
                        foreach ($ret as  $value) {
                           if(!empty($value)){
                             $total_profits += $value;
                                ...
                                echo '<th >'.Applib::format_currency($inv->currency, $value).'</th>';
                                // Sum of Revenue in Project & Month wise                      
                           }else{
                            echo '<td>-</td>';                          
                          }
                      ...
                      echo '<th style = "text-align:right">'.Applib::format_currency($inv->currency, $total_profits).'</th>';
                      echo '</tr>';
