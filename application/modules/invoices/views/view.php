<header class="header-fixed clearfix hidden-print p-0"><meta http-equiv="Content-Type" content="text/html; charset=utf-8">
                    <div class="row">
    ...
    <a href="<?=site_url()?>invoices/add_expenses/<?=$inv->client?>/<?=$inv->inv_id?>" class="btn btn-xs btn-danger" data-toggle="ajaxModal"><?=lang('expenses_available')?></a>
                        ...
                        <table id="inv-details" class="table sorted_table" type="invoices"><thead>
                            <tr>
                             ...
                             <?php //$test=   Invoice::has_items($inv->inv_id);
                                    ...
                                    <td>
                                        <?php if (User::is_admin() || User::perm_allowed(User::get_id(),'edit_all_invoices')) { ?>
                                            <a class="hidden-print"
                                               href="<?= base_url() ?>invoices/items/delete/<?=$item->item_id?>/<?=$item->invoice_id ?>/<?=$item->quantity?>" data-toggle="ajaxModal"><i class="fa fa-trash-o text-danger"></i>
                                            </a>
                                        <?php } ?>
                                    </td>
                                </tr>
