...
echo form_open('admin_settings/templates?settings=templates&group='.$template_group.'&email='.$setting_email, $attributes);
?>
    <div class="p-0">
                ...
                <div class="panel-body">
					<ul class="nav nav-tabs nav-tabs-solid m-b-20">
						...
						<li class="<?php if($setting_email == $temp){ echo "active"; } ?> ">
							<a href="<?=base_url()?>admin_settings/?settings=templates&group=<?=$template_group;?>&email=<?=$temp;?>"><?=lang($lang)?></a>
						</li>
					<?php endforeach; ?>
					</ul>
                    <input type="hidden" name="email_group" value="<?=$setting_email;?>">
                    <input type="hidden" name="return_url" value="<?=base_url()?>admin_settings/?settings=templates&group=<?=$template_group;?>&email=<?=$setting_email;?>">
