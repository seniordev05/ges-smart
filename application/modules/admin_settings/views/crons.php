					...
					<table id="cron-jobs" class="table table-striped custom-table m-b-0">
						...
						<tbody>
							...
							<tr>
								...
								?></td>
								<td class="text-right">
									<a data-rel="tooltip" data-original-title="<?=lang('toggle_enabled')?>" class="cron-enabled-toggle btn btn-xs btn-<?=($cron->enabled == 1 ? 'success':'default')?> m-r-xs" href="#" data-role="1" data-href="<?=base_url()?>admin_settings/hook/enabled/<?=$cron->module?>"><i class="fa fa-check"></i></a>
									<?php
