			...
			<table id="table-translations" class="table table-striped table-bordered custom-table m-b-0 AppendDataTables">
				...
				<tbody>
					...
					<tr>
						...
						<td class=""><a href="<?=base_url()?>admin_settings/translations/view/<?=$l->name?>/?settings=translations"><?=ucwords(str_replace("_"," ", $l->name))?></a></td>
						<td>
						...
						<td class=""><?=$total?></td>
						<td class="text-right">
						  <a data-rel="tooltip" data-original-title="<?=lang('backup')?>" class="backup-translation btn btn-xs btn-default" href="#" data-href="<?=base_url()?>admin_settings/translations/backup/<?=$l->name?>/?settings=translations"><i class="fa fa-download"></i></a>
						  <a data-rel="tooltip" data-original-title="<?=lang('restore')?>" class="restore-translation btn btn-xs btn-default" href="#" data-href="<?=base_url()?>admin_settings/translations/restore/<?=$l->name?>/?settings=translations"><i class="fa fa-upload"></i></a>
						  <a data-rel="tooltip" data-original-title="<?=($l->active == 1 ? lang('deactivate') : lang('activate') )?>" class="active-translation btn btn-xs btn-<?=($l->active == 0 ? 'default' : 'success' )?>" href="#" data-href="<?=base_url()?>admin_settings/translations/active/<?=$l->name?>/?settings=translations"><i class="fa fa-eye"></i></a>
						  <a data-rel="tooltip" data-original-title="<?=lang('edit')?>" class="btn btn-xs btn-info" href="<?=base_url()?>admin_settings/translations/view/<?=$l->name?>/?settings=translations"><i class="fa fa-edit"></i></a>
						</td>
					</tr>
					<?php endforeach; ?>
				</tbody>
			...
			<table id="table-translations-files" class="table table-striped table-bordered custom-table m-b-0 AppendDataTables">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td class=""><a href="<?=base_url()?>admin_settings/translations/edit/<?=$language?>/<?=$shortfile?>/?settings=translations"><?=$fn?></a></td>
                        <td>
                            <div class="progress progress-sm">
                            <?php $bar = 'danger'; if ($pc > 20) { $bar = 'warning'; } if ($pc > 50) { $bar = 'info'; } if ($pc > 80) { $bar = 'success'; } ?>
                            <div class="progress-bar progress-bar-<?=$bar?>" role="progressbar" aria-valuenow="<?=$pc?>" aria-valuemin="0" aria-valuemax="100" style="width: <?=$pc?>%;">
                        ...
                        <td class=""><?=$total?></td>
                        <td class="text-right">
                          <a class="btn btn-xs btn-default" href="<?=base_url()?>admin_settings/translations/edit/<?=$language?>/<?=$shortfile?>/?settings=translations"><i class="fa fa-edit"></i></a>
                        </td>
                    </tr>
                    <?php endforeach; ?>
                </tbody>
...
echo form_open_multipart('admin_settings/translations/save/'.$language.'/'.$language_file.'/?settings=translations', $attributes); ?> 
		...
		<div class="panel-heading">
			<div class="row">
				<div class="col-xs-6">
					...
					?>
					<?=lang('translations')?> | <a href="<?=base_url()?>admin_settings/translations/view/<?=$language?>/?settings=translations"><?=ucwords(str_replace("_"," ", $language))?></a> | <?=$fn?> | <?=$percent?>% <?=mb_strtolower(lang('done'))?>
				</div>
