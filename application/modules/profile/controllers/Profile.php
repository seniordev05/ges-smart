    ...
    if($_POST){
		...
		if($this->session->userdata('user_id') != 1){
			...
			$activity = $this->db->select('*')
			  ...
			  ->or_where('value2',$user_id)
			  ->where('module_field_id',0)
			  ->group_end()
...
$data['activity'] = $activity;
	...
	public function file_download($file_name = NULL){
		if($file_name){
			$this->load->helper('download');
			$file=realpath('./assets/uploads').'\\'.$file_name;
			if(file_exists($file)){
    ...
    public function delete($act_id = NULL)
    {
        if ($this->input->post()) {
        	$act_id=$this->input->post('act_id');
        	$act=$this->db->get_where('dgt_activities', array('activity_id' => $act_id))->result_array()[0];
        	$filename=$act['value1'];
        	// var_dump($send_user.','.$filename); exit;
        	$num=$this->db->get_where('dgt_activities', array('value1' => $filename))->num_rows();
        	if($num==1) { 
        		$path='./assets/uploads/'.$filename;
        		unlink($path);
        	}
			$this->db->where('activity_id' , $act_id);
		    $query = $this->db->delete(dgt_activities);            // Client::delete($company);
            redirect('profile/activities');
        } else {
            $data['act_id'] = $act_id;
