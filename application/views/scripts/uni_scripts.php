                ...
                $('.typeahead').bind('typeahead:select', function(ev, suggestion) {
                    $.ajax({
                        ...
                        success: function(response){
                            ...
                            $('#auto-unit-cost').val(response.selling_price);
                        }
                    });
                });
            }
...
function leave_days_calc(){
    ...
    var total_count = $('#total_leave_counts').val();
    if(total_count >= new_cnt){
        $('#req_leave_count').val(new_cnt);
        $('#employee_add_leave').removeAttr('disabled');
    }else{
        toastr.error('Not Allowed');
        $('#employee_add_leave').attr('disabled','disabled');
    }
        ...
        else
        {
         ...
         $.post('<?php echo base_url();?>payroll/status_change',{employeeid:id,status:status},function(data){
            ...
            if(data.status==1)
            {
              ...
              $(".payroll_total").val(data.payroll_total);
            }
            else
            {
...
<script type="text/javascript">
// $(document).ready(function(){
    // var all_uploadfilesize = $('#filesizeupload').val();
     $(document).on('change',".UpLoadFileSize",function() {
    // $('.UpLoadFileSize').on("change",function(){
        var f = this.files[0];
        // var r = bytesToSize(all_uploadfilesize);
        if (f.size > 8000000 || f.fileSize > 8000000)
        {
           //show an alert to the user
           // alert("Allowed file size exceeded. (Max. 8 MB)");
           toastr.error("Allowed file size exceeded. (Max. 8 MB)");
