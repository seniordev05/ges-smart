...
class Invoice extends CI_Model
{
    ...
    public static function saved_items_where()
    {
        return self::$db->where(array('quantity >' => 0))->get('items_saved')->result();
    }
    ...
    public static function allsum_cost_quant($invoice)
    {
        // return $invoice;
        // $sql = "SELECT * FROM dgt_items WHERE invoice_id = $invoice ORDER BY item_id DESC";
        // $latest = self::$db->query($sql)->result();
        // // return $latest;
        // if (count($invoices>1)) {
        //     return round($latest[0]->cost_quant,2);
        // }
       $allsum_cost_quant=self::$db->select_sum('cost_quant')->where('invoice_id', $invoice)
       ->where('paid_status',0)
       ->get('items')->row()->cost_quant;

        return round($allsum_cost_quant, 2);
    }

    public static function allsum_sale_quant($invoice)
    {
        // return $invoice;
        // $sql = "SELECT * FROM dgt_items WHERE invoice_id = $invoice ORDER BY item_id DESC";
        // $latest = self::$db->query($sql)->result();
        ...
        //     return round($latest[0]->cost_quant,2);
        // }
       $allsum_sale_quant=self::$db->select_sum('sale_quant')->where('invoice_id', $invoice)
       ->where('paid_status',0)
       ->get('items')->row()->sale_quant;

        return round($allsum_sale_quant, 2);
    }
